...
TEST_CASE("Bubble Проверка нулевого массива", "[bubble][unit]") {
	int arr[3] = {};			
	int ShouldBeArr[3] = {};	
	BubbleSort(arr, 3);
	REQUIRE(arrEqual(arr, ShouldBeArr, 3));
}
TEST_CASE("Bubble Проверка массива с одним элементом", "[bubble][unit]") {
	int arr[1] = {5};
	int ShouldBeArr[1] = {5};
	BubbleSort(arr, 1);
	REQUIRE(arrEqual(arr, ShouldBeArr, 1));
}
TEST_CASE("Bubble Проверка  отсортированного массива", "[bubble][unit]") {
	int arr[5] = { 1, 2, 3, 4, 5 };
	int ShouldBeArr[5] = { 1, 2, 3, 4, 5 };
	BubbleSort(arr, 5);
	REQUIRE(arrEqual(arr, ShouldBeArr, 5));
}
TEST_CASE("Bubble Проверка обратного массива", "[bubble][unit]") {
	int arr[5] = {5, 4, 3, 2, 1};
	int ShouldBeArr[5] = {1, 2, 3, 4, 5};
	BubbleSort(arr, 5);
	REQUIRE(arrEqual(arr, ShouldBeArr, 5));
}
TEST_CASE("Bubble Проверка массива double", "[bubble][unit]") {
